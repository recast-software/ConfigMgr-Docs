...
#$LicenseFile = "$($PSScriptRoot)\$((Get-ChildItem $PSScriptRoot\*.bin).Name)"
#$LicenseFileName = "$((Get-ChildItem $PSScriptRoot\*.bin).Name)"
...
if (!(test-path -path $RecastProgData)){New-Item $RecastProgData -ItemType Directory -Force}
...
#Run the Installer:  
CMTraceLog -Message "Starting Command: msiexec.exe /I ""$($installer)"" /passive" -Type 1 -LogFile $LogFile
$InstallProcess = Start-Process msiexec.exe -Wait -ArgumentList "/I ""$($installer)"" /passive" -Passthru
...
if ($InstallProcess.ExitCode -eq "0")
    ...
    #OPTINAL COPY OF 3rd Party Tools (Must be downloaded and in your source directory in a subfolder called 3rdPartyTools)
        ...
        <#IF No License File, Copy the one into place
        if (!(Test-Path -Path "$RecastProgData\Licenses\*.bin"))
            {
            if (Test-Path -Path "$RecastProgData\Licenses\*.license"){Remove-Item -Path "$RecastProgData\Licenses" -Force -Recurse}
            New-Item -Path "$RecastProgData\Licenses" -ItemType Directory -Force
            Copy-Item -Path $LicenseFile -Destination "$($RecastProgData)\Licenses\$($LicenseFileName)" -Verbose
            CMTraceLog -Message " $LicenseFile to $($RecastProgData)\Licenses\$($LicenseFileName)" -Type 1 -LogFile $LogFile
            }
        #>
    ...
    ForEach ($RecastRCTDir in $RecastRCTDIRs)
        ...
        #Else if there is a COnfigFile, Set Custom Settings
            ...
            if (Test-Path "$Recast3rdParty\PsExec.exe")
                {
                $ConfigurationXMLPsExec = Get-Content -path "$RecastRCTDir\configuration.xml" -Raw
                if ($ConfigurationXMLPsExec | Where-Object -FilterScript { $_ -match '<PsExecPath i:nil="true"/>'})
                    {
                ...
                else{CMTraceLog -Message " Interactive Command Prompt already set" -Type 1 -LogFile $LogFile}
                }      
            ...
            if (Test-Path "$Recast3rdParty\Explorer++.exe")
                {
                $ConfigurationXMLExplorer = Get-Content -path "$RecastRCTDir\configuration.xml" -Raw
                if ($ConfigurationXMLExplorer | Where-Object -FilterScript { $_ -match '<AlternateExplorerPath i:nil="true"/>'})
                    {
                ...
                else{CMTraceLog -Message " Alternate Explorer already set" -Type 1 -LogFile $LogFile}
                }      
